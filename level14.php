...
if(isset($_SESSION['email']))
{
	...
	if($queryarray['uid'] == $uid)
	{
		if(isset($_POST['ans']))
		{
			...
			if($ans == "3dcc9d053cf1fddf3a2d3646b437464a")
			{
				if($queryarray['lvl'] == 14)
				{
				date_default_timezone_set('Asia/Kolkata');
				$clear = date("d/m/Y H:i:s");
				$update = mysql_query("UPDATE illushun SET lclear='$clear', lvl=15 WHERE email='$email'");
				}
			echo "<script type='text/javascript'>setTimeout(\"window.location = 'level15.php'\",0);</script>";
...
<div id="ques">
One gets divided by his friend. Their integration will lead to a big revolution if it is done 

with due respect to the friend. Name it.
</div>
<div id="ans">
