...
if(isset($_SESSION['email']))
{
	...
	if($queryarray['uid'] == $uid)
	{
		if(isset($_POST['ans']))
		{
			...
			if($ans == "a853c1f60f30d0c07c8d528ee6c9c845")
			{
				if($queryarray['lvl'] == 17)
				{
				date_default_timezone_set('Asia/Kolkata');
				$clear = date("d/m/Y H:i:s");
				$update = mysql_query("UPDATE illushun SET lclear='$clear', lvl=18 WHERE email='$email'");
				}
			echo "<script type='text/javascript'>setTimeout(\"window.location = 'level18.php'\",0);</script>";
